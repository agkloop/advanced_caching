...
class CacheEntry:
    ...
    def __repr__(self) -> str:
        if self.fresh_until == float("inf"):
            expires = "never"
        else:
            expires = datetime.datetime.fromtimestamp(self.fresh_until).isoformat()
        age_s = round(self.age(), 1)
        return f"CacheEntry(value={self.value!r}, expires={expires}, age={age_s}s)"
...
class InstrumentedStorage(CacheStorage):
    ...
    def get_entry(self, key: str, now: float | None = None) -> CacheEntry | None:
        start = time.perf_counter()
        try:
            inner = getattr(self._storage, "get_entry")
            result = inner(key, now) if now is not None else inner(key)
            duration = time.perf_counter() - start
    ...
    def get_all_keys(self) -> list[str]:
        """Retrieve all keys from the underlying storage if supported.

        Returns empty list if not supported.
        """
        if hasattr(self._storage, "get_all_keys"):
